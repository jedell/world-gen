...
Grid *size_0_grid()
{
	...
	Vector3 icos_tiles[12] = {
		Vector3init(-x, c, z), Vector3init(x, c, z), Vector3init(-x, c, -z), Vector3init(x, c, -z),
		Vector3init(c, z, x), Vector3init(c, z, -x), Vector3init(c, -z, x), Vector3init(c, -z, -x),
		Vector3init(z, x, c), Vector3init(-z, x, c), Vector3init(z, -x, c), Vector3init(-z, -x, c)};

	int icos_tiles_n[12][5] = {
		{9, 4, 1, 6, 11}, {4, 8, 10, 6, 0}, {11, 7, 3, 5, 9}, {2, 7, 10, 8, 5}, {9, 5, 8, 1, 0}, {2, 3, 8, 4, 9}, {0, 1, 10, 7, 11}, {11, 6, 10, 3, 2}, {5, 3, 10, 1, 4}, {2, 5, 4, 0, 11}, {3, 7, 6, 1, 8}, {7, 2, 9, 0, 6}};
	...
	for (int i = 0; i < grid->corners.size(); i++)
	{
		Vector3 *v = &(grid->corners[i]).v;
		float mag = sqrt(pow(v->x, 2) + pow(v->y, 2) + pow(v->z, 2));
		float ratio = radius / mag;
...
Grid *_subdivided_grid(Grid *prev)
{
	...
	for (Corner &c : grid->corners)
	{
		for (int k = 0; k < 3; k++)
		{
			c.corners[k] = c.tiles[k]->corners[(position(c.tiles[k], &c) + 1) % (c.tiles[k]->edges.size())];
		}
	}
	// new edges
	int next_edge_id = 0;
...
void _add_corner(int id, Grid *grid, int t1, int t2, int t3)
{
	...
	Vector3 v = Vector3Add(Vector3Add(t[0]->v, t[1]->v), t[2]->v);
	c->v = Vector3Scale( Vector3Normalize(v), RADIUS );
	for (int i = 0; i < 3; i++)
	{
...
void _add_edge(int id, Grid *grid, int t1, int t2)
{
	...
	Corner *c[2] = {
		...
		&grid->corners[t[0]->corners[(position(t[0], t[1]) + 1) % t[0]->edges.size()]->id]};
	for (int i = 0; i < 2; i++)
	{
